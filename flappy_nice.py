...
# Load bird image now that a display mode is set
try:
    bird_image = pygame.image.load("bird_transparent.png").convert_alpha()
...
class Bird:
    ...
    def reset(self):
        ...
        self.y = HEIGHT // 2
        self.velocity = 0
        # Use bird.png image dimensions for collision if needed
        if bird_image:
    ...
    def draw(self, surface, score):
        # Draw the bird using the image if available
        if bird_image:
            surface.blit(bird_image, (int(self.x), int(self.y)))
        else:
            # Fallback: draw a circle
            pygame.draw.circle(surface, (min(255, 100 + int(score * 5) % 155), 100, 200), (int(self.x), int(self.y)), 20)
...
class Pipe:
    ...
    def collide(self, bird):
        # Simple collision: check if bird's rectangle intersects the pipe rectangles
        if bird.x + (bird.width if hasattr(bird, 'width') else 40) > self.x and bird.x < self.x + PIPE_WIDTH:
            if bird.y < self.top or bird.y + (bird.height if hasattr(bird, 'height') else 40) > self.bottom:
                return True
        return False
...
def main_menu():
    # Example of a classic scrolling background effect for the main menu.
    scroll_offset = 0
    menu_running = True
    while menu_running:
        screen.fill(BLACK)
        # Draw scrolling background (simple horizontal scrolling stripes)
        for i in range(0, WIDTH, 40):
            pygame.draw.rect(screen, (50, 50, 50), (i - scroll_offset, 0, 20, HEIGHT))
        scroll_offset = (scroll_offset + 1) % 40
        
        draw_text("Novel Flappy Bird Twist", 36, WHITE, (WIDTH//2, HEIGHT//4))
        draw_text("1. Train AI (DQN)", 28, WHITE, (WIDTH//2, HEIGHT//2 - 40))
        draw_text("2. Play with AI", 28, WHITE, (WIDTH//2, HEIGHT//2))
        draw_text("3. Play (Human)", 28, WHITE, (WIDTH//2, HEIGHT//2 + 40))
        draw_text("Press 1, 2, or 3", 24, WHITE, (WIDTH//2, HEIGHT//2 + 100))
...
def train_dqn():
    ...
    num_episodes = 50000  # Increased to 50,000 episodes
    max_steps = 1000
    episode_scores = []
    ...
    for ep in range(num_episodes):
        ...
        for step in range(max_steps):
            ...
            if (local_bird.y < 0 or 
                local_bird.y + (local_bird.height if hasattr(local_bird, 'height') else 40) > HEIGHT or 
...
def play_with_dqn():
    ...
    while running:
            ...
            for pipe in pipes:
                ...
                if pipe.collide(bird):
                    ...
                    if hit_sound and not TRAINING_MODE:
                        hit_sound.play()
            if bird.y < 0 or bird.y + (bird.height if hasattr(bird, 'height') else 40) > HEIGHT:
                game_over = True
                if hit_sound and not TRAINING_MODE:
                    hit_sound.play()
        ...
        if game_over:
            draw_text("Game Over!", 36, WHITE, (WIDTH//2, HEIGHT//2 - 20))
            draw_text("Press SPACE to return to Main Menu", 28, WHITE, (WIDTH//2, HEIGHT//2 + 20))
        pygame.display.update()
